...
void si446x_set_property( uint8_t GROUP, uint8_t NUM_PROPS, uint8_t START_PROP, ... )
{
    ...
    while(NUM_PROPS--)
    {
        Pro2Cmd[cmdIndex] = (uint8_t) va_arg (argList, uint16_t);
        DBG("might not work\n");
        cmdIndex++;
    }
    va_end(argList);
...
void si446x_pa_pwr_lvl(uint8_t lvl)
{
	Pro2Cmd[0] = SI446X_CMD_ID_SET_PROPERTY;
	Pro2Cmd[1] = SI446X_CMD_ID_GET_MODEM_STATUS;
	Pro2Cmd[2] = 0x01;
	Pro2Cmd[3] = SI446X_PROP_GRP_INDEX_PA_PWR_LVL;
	Pro2Cmd[4] = lvl;
