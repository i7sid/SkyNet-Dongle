...
__weak void swu_rx_callback(void);
...
static void swu_setup_tx(void) {
  ...
  if (tx_fifo_wr_ind != tx_fifo_rd_ind) //data to send, proceed
  {
    ...
    reference = LPC_TIMER->TC + BIT_LENGTH;
    for (i = 0; i != cnt_edges; i++) //recalculate toggle points...
      edge[i] = (edge[i] + reference) //... an adjust for the...
        & 0x3FFFFFFF; //... timer range

    /*
    LPC_TIMER->MR[3] = edge[0]; //load MR3
    LPC_TIMER->MCR = LPC_TIMER->MCR | (1 << 9); //enable interrupt on MR3 match
    LPC_TIMER->EMR = LPC_TIMER->EMR | (3 << 10); //enable toggle on MR3 match
*/
    LPC_TIMER->MR[2] = edge[0]; //load MR2
    LPC_TIMER->MCR = LPC_TIMER->MCR | (1 << 6); //enable interrupt on MR2 match
    LPC_TIMER->EMR = LPC_TIMER->EMR | (1 << 9) | (1 << 8); //enable toggle on MR2 match
  }
    ...
    if (edge_index == char_end_index) //the end of the char:
    {
      ...
      if (edge_index == last_edge_index) //is this the last toggle?
          //TX_ISR_TIMER->EMR = 0x000003FF; //no more toggle on MAT3
      	  TX_ISR_TIMER->EMR = 0xFF3; //no more toggle on MAT2
      	  edge_index++; //update the edge index
      	  TX_ISR_TIMER->MR[2] = edge[edge_index]; //prepare the next toggle event
      	  //TX_ISR_TIMER->MR[3] = edge[edge_index]; //prepare the next toggle event
    }
...
//#define TIMER2_IRQHandler
void TIMER2_IRQHandler(void) {
	swu_isr_tx(LPC_TIMER);
}
...
******************************************************************************/
/*
void swu_isr_rx(LPC_TIMER_T* const RX_ISR_TIMER) {
  ...
  if (0 != (RX_ISR_TIMER->IR & 0x10)) //capture interrupt occured:
  {
    ...
    if ((swu_status & RX_ACTIVE) == 0) { //sw UART not active (start):
      edge_last = (signed long int) RX_ISR_TIMER->CR[0]; //initialize the last edge
      ...
      edge_current = (signed long int) RX_ISR_TIMER->CR[0]; //initialize the current edge
...
**
******************************************************************************/
void swu_init(LPC_TIMER_T* const UART_TIMER) {
  ...
  uint32_t clkin = Chip_Clock_GetPeripheralClockRate(SYSCTL_PCLK_TIMER2);
  BIT_LENGTH = clkin / 9600;
  DBG("bit_length: %d\n", BIT_LENGTH);
  ...
  */
  ...
  UART_TIMER->IR = 0x03F; //clear all TIMER0 flags
  UART_TIMER->PR = 0x00000000; //no prescaler
  UART_TIMER->MR[0] = 0x3FFFFFFF; //TIMER0_32 counts 0 - 0x3FFFFFFF
  UART_TIMER->MCR = 2; //reset TIMER0_32 on MR0
  //UART_TIMER->EMR = 0x0008; //drive MAT0.3 high
  UART_TIMER->EMR = 0x4; //drive MAT2.2 high
  ...
  while (0 == (LPC_GPIO1->PIN & (1 << PIN_SW_RX)))
