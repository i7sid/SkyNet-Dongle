...
/*
 * // TODO DEFECT on Arduino
 * //      (Auspacken von extheader scheitert an *last_ptr = hdr;)
 *
...
uint16_t mac_frame_data_unpack(mac_frame_data *frame, uint8_t *buffer, uint16_t length) {
  ...
  while (next_ext_type.type_length.type != EXTHDR_NO) {
    ...
    Serial.println((int)hdr, HEX);
    Serial.println((int)frame->extheader, HEX);
    if (hdr == NULL) {
      // TODO throw error?
      return pos;
    }
    ...
    *last_ptr = hdr;
    Serial.println((int)(hdr->next), HEX);
    last_ptr = &(hdr->next);
    Serial.println((int)last_ptr, HEX);
  }
  Serial.println("-");
